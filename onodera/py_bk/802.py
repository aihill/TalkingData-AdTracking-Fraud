...
def multi(argv):
    ...
    train.sample(frac=0.1, random_state=seed).to_pickle('../data/X_train{}.p'.format(i))
    y.sample(frac=0.1, random_state=seed).to_pickle('../data/y_train{}.p'.format(i))

cnt = 0
proc = 2
for i in range(5):
    train_seeds = np.random.randint(99999, size=2)
    pool = Pool(proc)
    pool.map(multi, zip([cnt, cnt+1],train_seeds))
    pool.close()
    cnt += proc
