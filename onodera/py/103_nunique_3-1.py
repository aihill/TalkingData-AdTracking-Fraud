...
comb = []
tmp1 = list(combinations(['ip', 'app', 'device', 'os', 'channel'], 3))
for c1 in tmp1:
...
# train
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/103__*_train.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/103-1_train', utils.SPLIT_SIZE)

gc.collect()

# test
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/103__*_test.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/103-1_test', utils.SPLIT_SIZE)
...
# train
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/103__*_train.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/103-2_train', utils.SPLIT_SIZE)

gc.collect()

# test
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/103__*_test.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/103-2_test', utils.SPLIT_SIZE)
...
# train
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/103__*_train.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/103-3_train', utils.SPLIT_SIZE)

gc.collect()

# test
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/103__*_test.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/103-3_test', utils.SPLIT_SIZE)
