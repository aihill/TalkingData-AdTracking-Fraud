...
def multi(count_keys):
    ...
    result.iloc[0:utils.TRAIN_SHAPE].to_pickle('../data/003__{}_train.p'.format(count_keys_))
    result.iloc[utils.TRAIN_SHAPE:].to_pickle('../data/003__{}_test.p'.format(count_keys_))
...
utils.to_pickles(df, '../data/003_train', utils.SPLIT_SIZE)


# test
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/003__*_test.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/003_test', utils.SPLIT_SIZE)
