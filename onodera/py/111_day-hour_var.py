...
# train
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/111__*_train.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/111-2_train', utils.SPLIT_SIZE)

gc.collect()

# test
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/111__*_test.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/111-2_test', utils.SPLIT_SIZE)
...
# train
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/111__*_train.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/111-3_train', utils.SPLIT_SIZE)

gc.collect()

# test
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/111__*_test.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/111-3_test', utils.SPLIT_SIZE)
