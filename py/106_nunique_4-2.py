...
utils.to_pickles(df, '../data/106-1_train', utils.SPLIT_SIZE)

gc.collect()

# test
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/106__*_test.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/106-1_test', utils.SPLIT_SIZE)
...
# train
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/106__*_train.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/106-2_train', utils.SPLIT_SIZE)

gc.collect()

# test
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/106__*_test.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/106-2_test', utils.SPLIT_SIZE)
...
# train
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/106__*_train.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/106-3_train', utils.SPLIT_SIZE)

gc.collect()

# test
df = pd.concat([pd.read_pickle(f) for f in sorted(glob('../data/106__*_test.p'))], axis=1).reset_index(drop=True)
utils.to_pickles(df, '../data/106-3_test', utils.SPLIT_SIZE)
