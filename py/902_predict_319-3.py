...
# setting
submit_file_path = '../output/319-3.csv.gz'
SEED = 48
LOOP = 3
nround = 850
exe_submit = False
...
for keys in tqdm(utils.comb):
    gc.collect()
    keys_ = '-'.join(keys)
    test = pd.merge(test, pd.read_pickle('../data/{}_count_old.p'.format(keys_)), 
                     on=keys, how='left')
    test = pd.merge(test, pd.read_pickle('../data/{}_timestd_old.p'.format(keys_)), 
                     on=keys, how='left')
